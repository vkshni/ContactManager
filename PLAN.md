
# PROJECT TIMELINE

## Day 1 - Planning & Architect Design
- **Create directory structure** ✅
- **Data storage format, data constraints and rules** ✅
- **Features to be added** ✅
- **Create `DESIGN.md` file** ✅

## Day 2 - Storage Layer and Entity layer
- **Create Low level JSON handler** ✅
- **Create add and view methods** ✅
- **Create `Contact` entity using classes** ✅
- **Add `from_dict()` and `to_dict()` methods in entity** ✅

## Day 3 - Engine layer and CLI layer - I
- **Create `update()` and `delete()` methods in `storage.py`** ✅
- **Create `engine.py` and add methods `add_contact()` and `list_all`** ✅
- **Create `main.py` (CLI layer) and add command `list` and `help`** ✅

## Day 4 - Engine and CLI - II
- **Create `edit()`, `search()` and `delete()` methods in `engine.py`** ✅
- **Create validators in `entity.py`** ✅
- **Create `add` command in CLI** ✅